...
Because the scope.md contract is part of the versioned conventions, the assertion needs a minted convention base. It was first cut as `20260901-1907`; that cut was never published, and this release's `20260903-1825` carries the assertion instead, superseding both it and `20260724-1910`. Existing scopes keep their pins and behave exactly as before -- the index and dashboard read the field wherever it appears, so a stale mark works on older-pinned scopes too, but the documented contract lives in the new version, which new scopes pin automatically.

**Changed**
- Convention base minted as `20260903-1825/`: scope.md gains the optional `status: stale` lifecycle assertion (the one carve-out from descriptors-carry-no-state); `#docket` replaces three folder-types (kept as `#deprecated` with their anchors); new `#docket-mechanics` (`#records`, `#mixins`, `#triggers`, `#signals`, `#fires`, `#actors`, `#channels`, `#grants`, `#schedule-modes`, `#dispatcher`); the derived-location rule reworded; migrations gain the scope-by-scope journal rule.
