...
- If the user had `reminders` or `inbox` installed: run `setup-docket` to generate their personal `<username>-docket` skill, which covers both. It replaces the old generic skills with a per-user skill that knows their docket's location and preferences; their old reminder and inbox files are picked up by the docket migration the library-updater offers, not by the new skill.
...
The generic reminders, inbox, and writing-styles skills have been replaced by personal per-user skills generated by the setup skills (setup-docket, setup-writing-styles). Your personal skill names follow the pattern <username>-docket, <username>-writing-styles.
```
