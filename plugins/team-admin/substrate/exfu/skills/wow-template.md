...
- `{{username}}-docket` (reminders, frictionless capture, and things left for agents -- generated by `setup-docket`)
...
- The substrate follows the ExFu scope-based layout: three top-level entries at the root.
  - `exfu/` -- plugin-owned scope: the versioned contract at `exfu/<version>/ontology.md`, unversioned shipped content beside it (`readme.md`, `principles.md`, `librarians/`, `skills/`), derived data at `exfu/derived/`, version pointer at `exfu/latest.txt` (names the current `<version>`).
...
- `exfu/derived/agent-registry.json` -- librarian health and run history.
