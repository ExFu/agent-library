...
```
exfu/
  latest.txt              # single line: the current convention version (e.g. "20260903-1825")
...
- Dates, progress tracking, ambient status snapshots (those belong in `docket/` or the global index -- the declared `status: stale` assertion above is the one exception, because it only changes when someone changes it)
...
Follows: exfu/20260903-1825/ontology.md#context
```
