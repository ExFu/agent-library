...
2. Read `ledger/migrations.md`. Every id recorded there -- with any outcome, including `not-applicable` -- has been dealt with.
3. Pending = shipped minus recorded, in id order.

**If `ledger/` does not exist,** this is a library from before the ledger convention. Do not treat that as "everything is pending". Create the ledger, then continue to step 2, which decides what genuinely applies by looking at the library rather than at the absent record.
...
5. Record the outcome in `ledger/migrations.md` **before starting the next one**. A half-applied run that stops must leave an accurate record of exactly how far it got.
...
Append to `ledger/migrations.md`:
...
- Reads: the shipped migrations, `ledger/migrations.md`, and whatever each migration's `applies_when` inspects
- Writes: `ledger/migrations.md` always; each migration writes whatever its own frontmatter declares
